...
def execute(cmd, args):
	...
	global USER_NAME, USER_PRK, USER_PATH, LOGIN_IN
	...
	if cmd == 'help':
		...
		if args[0] != '-r':
			...
			status, info = file_system.rm(USER_NAME, USER_PATH, USER_PRK ,args)
			if not status:
			...
			status, info = file_system.rm_r(USER_NAME, USER_PATH, USER_PRK, args)
			if not status:
		...
		status, info = file_system.upload(USER_NAME, USER_PATH, USER_PRK, args)
		...
		status, info = file_system.download(USER_NAME, USER_PATH, USER_PRK, args)

		if not status:
			print 'Fail to download file %s because %s.' %(args[0], info)
			return False
		else:
			print 'Download file %s successfully, already at %s .' %(args[0], args[1])
			return True
		...
		status, info = file_system.mv(USER_NAME, USER_PATH, USER_PRK, args)
		...
		status, info = file_system.cp(USER_NAME, USER_PATH, USER_PRK, args)
